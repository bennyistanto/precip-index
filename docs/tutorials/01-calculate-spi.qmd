...
input_file = repo_root / 'input' / 'terraclimate_bali_ppt_1958_2024.nc'
...
By default, SPI uses the **Gamma** distribution. You can select alternative distributions via the `distribution` parameter:

```python
# SPI with Pearson III distribution
spi_3_p3 = spi(precip, scale=3, distribution='pearson3')

# SPI with GEV for extreme value analysis
spi_3_gev = spi(precip, scale=3, distribution='gev')
```

Available options: `'gamma'` (default), `'pearson3'`, `'log_logistic'`, `'gev'`, `'gen_logistic'`. See [Probability Distributions](../technical/distributions.qmd) for guidance on when to use each.
:::
...
plt.tight_layout()
plt.savefig(output_plots / 'spi_3_timeseries.png', dpi=150, bbox_inches='tight')
plt.show()
```
...
plt.tight_layout()
plt.savefig(output_plots / 'spi_multi_comparison.png', dpi=150, bbox_inches='tight')
plt.show()
```
...
save_index_to_netcdf(spi_3, output_netcdf / 'spi_3_bali.nc', compress=True)
print(f"✓ Saved SPI-3 to {output_netcdf / 'spi_3_bali.nc'}")

# Save multi-scale SPI
save_index_to_netcdf(spi_multi, output_netcdf / 'spi_multi_scale_bali.nc', compress=True)
print(f"✓ Saved multi-scale SPI to {output_netcdf / 'spi_multi_scale_bali.nc'}")
```
...
save_fitting_params(
    params,
    output_netcdf / 'spi_gamma_params_12_month_bali.nc',
    scale=12,
    periodicity='monthly',
    index_type='spi'
)

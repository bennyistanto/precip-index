...
2. **Temperature + latitude (Thornthwaite)** — pass `temperature=` and `latitude=`; PET is computed internally using the **Thornthwaite method** (requires only mean temperature)
3. **Temperature + Tmin/Tmax + latitude (Hargreaves)** — pass `temperature=`, `temp_min=`, `temp_max=`, and `latitude=`; PET is computed using the **Hargreaves-Samani method** (better for arid/semi-arid regions)

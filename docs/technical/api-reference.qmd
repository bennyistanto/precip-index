...
| `temperature` | xarray.DataArray | No | None | Temperature for PET calculation (if pet not provided) |
...
# Auto-compute PET from temperature
...
| Parameter | Type | Required | Default | Description |
| ----------- | ------ | ---------- | --------- | ------------- |
| `temperature` | xarray.DataArray or numpy.ndarray | Yes | - | Monthly temperature data (°C) |
...
pet = calculate_pet(temp, latitude=lat, data_start_year=1958)
```

...
The `plot_event_timeline()` function shows dry/wet events as horizontal bars with severity indicated by color intensity.

![Event timeline showing identified dry/wet events along the time axis. Bar height and color indicate event severity and duration.](../images/test_plot_timeline.png){#fig-plot-timeline}

### 3.4 Distribution Comparison (Run Theory)

Event characteristics (duration, magnitude, intensity) can differ depending on which distribution was used to compute the SPI. This plot compares event characteristics across distributions.

![Run theory event comparison across distributions, showing how the choice of probability distribution affects event identification.](../images/test_runtheory_distribution_comparison.png){#fig-runtheory-comparison}
...
The `calculate_period_statistics()` function counts events per grid cell over a defined period. The spatial map below shows dry (drought) event counts for 2000--2023.

![Spatial map of dry event counts (SPI-12 < -1.2, minimum 2-month duration) for the period 2000--2023 across Bali.](../images/test_spatial_drought_events.png){#fig-spatial-drought}
